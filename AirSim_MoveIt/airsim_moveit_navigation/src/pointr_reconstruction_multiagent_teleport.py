...
client.takeoffAsync(5, "drone_1").join()
client.takeoffAsync(5, "drone_2").join()
client.rotateToYawAsync(-90, 3, 5, "drone_1").join()
client.rotateToYawAsync(-90, 3, 5, "drone_2").join()
...
prev_candidate_distance_threshold = 8
...
drone_1_ned_position = PoseStamped()
drone_2_ned_position = PoseStamped()
...
def odom_callback(data):
    ...
    global drone_1_ned_position
    drone_1_ned_position = data.pose
    ...
    drone_1_pointr_position = tf2_geometry_msgs.do_transform_pose(data.pose, trans)
    '''
...
def odom_2_callback(data):
    ...
    data.pose.pose.position.x -= 4

    global drone_2_ned_position
    drone_2_ned_position = data.pose
    '''
    try:
        trans = tf_buffer.lookup_transform("pointr", data.header.frame_id,
                                           data.header.stamp,
    ...
    drone_2_pointr_position = tf2_geometry_msgs.do_transform_pose(data.pose, trans)
    '''
...
def listener():
    ...
    candidates_visited = np.zeros(3).reshape(1, 3)
    nbv_poses = np.zeros(3).reshape(1, 3)
    ...
    while True:
        ...
        drone_1_trajectories = np.array([[0,0,0,0,0]])
        drone_2_trajectories = np.array([[0,0,0,0,0]])
        ...
        for item in candidate_poses:
            rospy.loginfo("Checking if Drones can fly to candidate viewpoint [%d/%d]", candidate_count, candidate_poses.shape[0])
            candidate_count += 1
            candidate_too_close = False
            if not first_candidate:
                for visited in candidates_visited:
                    visited_scaled = visited * m
                    visited_scaled += centroid
                    ...
                    dist_to_previous_candidate = np.linalg.norm(item - visited_scaled)
            ...
            rospy.loginfo("Getting RRT path for Drone 1")
            ...
            if(rrt_poses[0][0] == -99 and rrt_poses[0][1] == -99 and rrt_poses[0][2] == -99):
                rospy.loginfo("No valid RRT path for Drone 1, continuing")
                continue

            drone_1_rrt_poses = copy.deepcopy(rrt_poses)

            rospy.loginfo("Getting RRT path for Drone 2")
            point = Point(item[0], item[1], item[2])
            orientation = Quaternion(0, 0, 0, 2)
            pose = Pose(point, orientation)
            global rrt_path
            rrt_path = False
            cand_pub.publish(pose)
            ...
            global rrt_poses
            if(rrt_poses[0][0] == -99 and rrt_poses[0][1] == -99 and rrt_poses[0][2] == -99):
                rospy.loginfo("No valid RRT path for Drone 2, continuing")
                continue

            drone_2_rrt_poses = copy.deepcopy(rrt_poses)
            ...
            if first_check:
                ...
                pose_number = np.empty([drone_1_rrt_poses.shape[0], 1])
                pose_number.fill(valid_count)

                drone_number = np.empty([drone_1_rrt_poses.shape[0], 1])
                drone_number.fill(0)

                drone_1_trajectories = np.concatenate((drone_number, pose_number, drone_1_rrt_poses), axis=1)

                pose_number = np.empty([drone_2_rrt_poses.shape[0], 1])
                pose_number.fill(valid_count)

                drone_number = np.empty([drone_2_rrt_poses.shape[0], 1])
                drone_number.fill(1)

                drone_2_trajectories = np.concatenate((drone_number, pose_number, drone_2_rrt_poses), axis=1)


                trajectories = np.concatenate((drone_1_trajectories, drone_2_trajectories), axis=0)
                ...
                trajectories = np.concatenate((trajectories, drone_1_trajectories, drone_2_trajectories), axis=0)

            valid_count = valid_count + 1

        if (valid_poses_drone_1_count == 0 or (valid_poses_drone_2_count == 0)):
            rospy.loginfo("One drone's valid candidate pose list is 0 [%d, %d], exiting", valid_poses_drone_1_count, valid_poses_drone_2_count)
            return
        ...
        try:
            trans = tf_buffer.lookup_transform("pointr", "world_ned",
                                           rospy.Time.now(),
                                           rospy.Duration(1))
        except tf2.LookupException as ex:
            rospy.logwarn(ex)
        ...
        global drone_1_ned_position
        global drone_2_ned_position
        drone_1_pointr_position = tf2_geometry_msgs.do_transform_pose(drone_1_ned_position, trans)
        drone_2_pointr_position = tf2_geometry_msgs.do_transform_pose(drone_2_ned_position, trans)

        drone_1_pose = np.array((drone_1_pointr_position.pose.position.x, drone_1_pointr_position.pose.position.y, drone_1_pointr_position.pose.position.z)).reshape(1, 3)
        drone_2_pose = np.array((drone_2_pointr_position.pose.position.x, drone_2_pointr_position.pose.position.y, drone_2_pointr_position.pose.position.z)).reshape(1, 3)

        drone_1_pose -= centroid
        drone_2_pose -= centroid
        drone_1_pose /= m
        drone_2_pose /= m
        
        global_pos = np.append(drone_1_pose, drone_2_pose, axis=0)
        np.save('/home/user/drone_pointr_positions.npy', global_pos)
        ...
        os.system('python3 /home/user/Code/nbv_simulation/find_multiagent_nbv_optimal_airsim.py')
        ...
        if first_candidate:
            ...
            candidates_visited[0][0] = pose_1[0]
            candidates_visited[0][1] = pose_1[1]
            candidates_visited[0][2] = pose_1[2]
            
            nbv_poses[0][0] = pose_1[0]
            nbv_poses[0][1] = pose_1[1]
            nbv_poses[0][2] = pose_1[2]

            candidates_visited = np.concatenate((candidates_visited, pose_2.reshape(1,3)), axis=0)
            nbv_poses = np.concatenate((nbv_poses, pose_2.reshape(1,3)), axis=0)

            np.save('/home/user/Data/Reconstruction/visited_poses.npy', nbv_poses)
            #np.save('/home/user/Data/Reconstruction/candidates_visited_poses.npy', candidates_visited)
            #return
        else:
            candidates_visited = np.concatenate((candidates_visited, pose_1.reshape(1,3)), axis=0)
            candidates_visited = np.concatenate((candidates_visited, pose_2.reshape(1,3)), axis=0)

            nbv_poses = np.concatenate((nbv_poses, pose_1.reshape(1,3)), axis=0)
            nbv_poses = np.concatenate((nbv_poses, pose_2.reshape(1,3)), axis=0)
        ...
        initial_point_1 = np.array((0, 0, 0))
        next_point_1 = np.array((0, 0, 0))
        initial_point_2 = np.array((0, 0, 0))
        next_point_2 = np.array((0, 0, 0))
        

        for i in range(start_index, max_traj):
            if i == start_index:
                initial_point_1[0] = drone_1_trajectory[0][2]
                initial_point_1[1] = drone_1_trajectory[0][3]
                initial_point_1[2] = drone_1_trajectory[0][4]

                initial_point_2[0] = drone_2_trajectory[0][2]
                initial_point_2[1] = drone_2_trajectory[0][3]
                initial_point_2[2] = drone_2_trajectory[0][4]

            else:
                initial_point_1[0] = next_point_1[0]
                initial_point_1[1] = next_point_1[1]
                initial_point_1[2] = next_point_1[2]

                initial_point_2[0] = next_point_2[0]
                initial_point_2[1] = next_point_2[1]
                initial_point_2[2] = next_point_2[2]

            if i < drone_1_trajectory.shape[0]:
                next_point_1[0] = drone_1_trajectory[i][2]
                next_point_1[1] = drone_1_trajectory[i][3]
                next_point_1[2] = drone_1_trajectory[i][4]
                #print("drone_1 initial point: [%.2f, %.2f, %2.f]" % (initial_point_1[0], initial_point_1[1], initial_point_1[2]))
                #print("drone_1 next point: [%.2f, %.2f, %2.f]" % (next_point_1[0], next_point_1[1], next_point_1[2]))
                distance = np.linalg.norm(next_point_1 - initial_point_1)
                #print('Distance of current drone_1 RRT path [%.2f]' % distance)
                total_flight_distance += distance

            if i < drone_2_trajectory.shape[0]:
                next_point_2[0] = drone_2_trajectory[i][2]
                next_point_2[1] = drone_2_trajectory[i][3]
                next_point_2[2] = drone_2_trajectory[i][4]
                #print("drone_2 initial point: [%.2f, %.2f, %2.f]" % (initial_point_2[0], initial_point_2[1], initial_point_2[2]))
                #print("drone_2 next point: [%.2f, %.2f, %2.f]" % (next_point_2[0], next_point_2[1], next_point_2[2]))
                distance = np.linalg.norm(next_point_2 - initial_point_2)
                #print('Distance of current drone_2 RRT path [%.2f]' % distance)
                total_flight_2_distance += distance

        final_index = drone_1_trajectory.shape[0] - 1
        rospy.loginfo("Drone 1 Traj Step[1]: [%.2f, %.2f, %.2f]", drone_1_trajectory[final_index][2], drone_1_trajectory[final_index][3], drone_1_trajectory[final_index][4])
        ...
        try:
            trans = tf_buffer.lookup_transform("world_ned", "world_enu",
                                           rospy.Time.now(),
                                           rospy.Duration(1))
        except tf2.LookupException as ex:
            rospy.logwarn(ex)
        ...
        pose_transformed = tf2_geometry_msgs.do_transform_pose(drone_1_goal, trans)
        ...
        client.simPause(False)
        client.reset()
        drone_1_pose = client.simGetVehiclePose("drone_1")
        drone_1_pose.position.x_val = pose_transformed.pose.position.x
        drone_1_pose.position.y_val = pose_transformed.pose.position.y
        drone_1_pose.position.z_val = pose_transformed.pose.position.z

        client.simSetVehiclePose(drone_1_pose, True, "drone_1")
        client.enableApiControl(True, "drone_1")
        client.armDisarm(True, "drone_1")
        client.takeoffAsync(5, "drone_1").join()
        client.moveToPositionAsync(drone_1_pose.position.x_val, drone_1_pose.position.y_val, drone_1_pose.position.z_val, 5, "drone_1").join()

        drone_1_goal.pose.position.x = centroid[0]
        drone_1_goal.pose.position.y = centroid[1]
        drone_1_goal.pose.position.z = centroid[2]
        try:
            trans = tf_buffer.lookup_transform("world_ned", "pointr",
                                           rospy.Time.now(),
                                           rospy.Duration(1))
        except tf2.LookupException as ex:
            rospy.logwarn(ex)
        ...
        center_transformed = tf2_geometry_msgs.do_transform_pose(drone_1_goal, trans)
        direction_y = center_transformed.pose.position.y - pose_transformed.pose.position.y
        direction_x = center_transformed.pose.position.x - pose_transformed.pose.position.x
        angle = math.atan2(direction_y, direction_x)

        angle = angle * 180 / math.pi

        client.rotateToYawAsync(angle, 3, 5, "drone_1").join()

        #drone_1_command_pub.publish(drone_1_goal)

        final_index = drone_2_trajectory.shape[0] - 1
        rospy.loginfo("Drone 2 Traj Step[1]: [%.2f, %.2f, %.2f]", drone_2_trajectory[final_index][2], drone_2_trajectory[final_index][3], drone_2_trajectory[final_index][4])
        ...
        pose_transformed = tf2_geometry_msgs.do_transform_pose(drone_2_goal, trans)

        pose_transformed.pose.position.x -= 4

        global drone_2_traversed
        drone_2_traversed = False

        drone_2_pose = client.simGetVehiclePose("drone_2")
        drone_2_pose.position.x_val = pose_transformed.pose.position.x
        drone_2_pose.position.y_val = pose_transformed.pose.position.y
        drone_2_pose.position.z_val = pose_transformed.pose.position.z

        client.simSetVehiclePose(drone_2_pose, True, "drone_2")
        client.enableApiControl(True, "drone_2")
        client.armDisarm(True, "drone_2")
        client.takeoffAsync(5, "drone_2").join()
        client.moveToPositionAsync(drone_2_pose.position.x_val, drone_2_pose.position.y_val, drone_2_pose.position.z_val, 5, "drone_2").join()

        drone_2_goal.pose.position.x = centroid[0]
        drone_2_goal.pose.position.y = centroid[1]
        drone_2_goal.pose.position.z = centroid[2]
        try:
            trans = tf_buffer.lookup_transform("world_ned", "pointr",
                                           rospy.Time.now(),
                                           rospy.Duration(1))
        ...
        center_transformed = tf2_geometry_msgs.do_transform_pose(drone_2_goal, trans)
        center_transformed.pose.position.x -= 4
        ...
        client.rotateToYawAsync(angle, 3, 5, "drone_2").join()
