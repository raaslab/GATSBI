...
bool findSegmentation(int x)
{
    //ROS_INFO("Segmentation Value: %d", x);
    return x == 20;
}
        ...
        for (auto& curr_sensor_map : vehicle_setting->sensors) {
            ...
            if (sensor_setting->enabled) {
                ...
                case SensorBase::SensorType::Lidar: {
                    ...
                    //sensor_publisher.publisher = nh_private_.advertise<sensor_msgs::PointCloud2>(curr_vehicle_name + "/lidar/LidarCustomTrimmed", 10);
                    ROS_INFO("Current vehicle name: %s", curr_vehicle_name.c_str());
                    if(count == 0)
                        segmented_publisher = nh_private_.advertise<sensor_msgs::PointCloud2>(curr_vehicle_name + "/lidar/LidarCustomTrimmed", 10);
                    else if(count == 1)
                        segmented_publisher_2 = nh_private_.advertise<sensor_msgs::PointCloud2>(curr_vehicle_name + "/lidar/LidarCustomTrimmed", 10);
                    break;
                }
                default: {
...
// look at UnrealLidarSensor.cpp UnrealLidarSensor::getPointCloud() for math
// read this carefully https://docs.ros.org/kinetic/api/sensor_msgs/html/msg/PointCloud2.html
sensor_msgs::PointCloud2 AirsimROSWrapper::get_lidar_segmentation_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& vehicle_name) const
{
    sensor_msgs::PointCloud2 lidar_msg;
    lidar_msg.header.stamp = ros::Time::now();
    lidar_msg.header.frame_id = vehicle_name;
    ...
    if (lidar_data.point_cloud.size() > 3) {
        std::vector<int> data_std = lidar_data.segmentation;
        std::vector<float> data_std_trimmed;

        std::vector<int>::iterator iter = data_std.begin();

        int count = 0;
        int count_points = 0;
        while ((iter = std::find_if(iter, data_std.end(), findSegmentation)) != data_std.end())
        {
            count = std::distance(data_std.begin(), iter);
            count_points = count * 3;
            data_std_trimmed.push_back(lidar_data.point_cloud[count_points]);
            data_std_trimmed.push_back(lidar_data.point_cloud[count_points + 1]);
            data_std_trimmed.push_back(lidar_data.point_cloud[count_points + 2]);
            iter++;
        }

        //ROS_INFO("Total points in segmentation: %d", count_points);

        lidar_msg.height = 1;
        lidar_msg.width = data_std_trimmed.size() / 3;
        ...
        const unsigned char* bytes = reinterpret_cast<const unsigned char*>(data_std_trimmed.data());
        vector<unsigned char> lidar_msg_data(bytes, bytes + sizeof(float) * data_std_trimmed.size());
        lidar_msg.data = std::move(lidar_msg_data);
        ...
        for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
            if (!vehicle_name_ptr_pair.second->lidar_pubs.empty()) {
                for (auto& lidar_publisher : vehicle_name_ptr_pair.second->lidar_pubs) {
                    ...
                    if(count == 0) {
                        sensor_msgs::PointCloud2 lidar_trimmed_msg = get_lidar_segmentation_from_airsim(lidar_data, vehicle_name_ptr_pair.first);
                        segmented_publisher.publish(lidar_trimmed_msg);
                    }
                    else if(count == 1) {
                        sensor_msgs::PointCloud2 lidar_trimmed_msg = get_lidar_segmentation_from_airsim(lidar_data, vehicle_name_ptr_pair.first);
                        segmented_publisher_2.publish(lidar_trimmed_msg);
                    }
                                                                  ...
                                                                  const std::string frame_id)
{
    ...
    img_msg_ptr->header.stamp = ros::Time::now(); //airsim_timestamp_to_ros(img_response.time_stamp);
                                                                        ...
                                                                        const std::string frame_id)
{
    ...
    depth_img_msg->header.stamp = ros::Time::now(); //airsim_timestamp_to_ros(img_response.time_stamp);
...
void AirsimROSWrapper::process_and_publish_img_response(const std::vector<ImageResponse>& img_response_vec, const int img_response_idx, const std::string& vehicle_name)
{
    ...
    for (const auto& curr_img_response : img_response_vec) {
        ...
        camera_info_msg_vec_[img_response_idx_internal].header.stamp = ros::Time::now(); //airsim_timestamp_to_ros(curr_img_response.time_stamp);
...
void AirsimROSWrapper::publish_camera_tf(const ImageResponse& img_response, const ros::Time& ros_time, const std::string& frame_id, const std::string& child_frame_id)
{
    ...
    cam_tf_body_msg.header.stamp = ros::Time::now(); //airsim_timestamp_to_ros(img_response.time_stamp);
    ...
    cam_tf_optical_msg.header.stamp = ros::Time::now(); //airsim_timestamp_to_ros(img_response.time_stamp);
