...
typedef actionlib::SimpleActionServer<dji_sdk_demo::NavigationAction> Server;

void execute(const dji_sdk_demo::NavigationGoalConstPtr& goal, Server* as) 
{


    ROS_INFO("execute ran\n x:%f, y:%f, z:%f\n", goal->goal_pose.position.x, goal->goal_pose.position.y, goal->goal_pose.position.z);
    mission.state = 1;

    mission.reset();
    mission.start_gps_location = current_gps;
    mission.start_local_position = current_local_pos;
    double yaw = tf::getYaw(goal->goal_pose.orientation);
    yaw = yaw * 180 / 3.14159;
    ROS_INFO("Yaw: %f", yaw);
    mission.setTarget(goal->goal_pose.position.x, goal->goal_pose.position.y, goal->goal_pose.position.z, yaw);
    //ROS_INFO(tf::getYaw(goal->goal_pose.orientation));
    mission.step();
...
{
	printf("server ran\n");
	ros::init(argc, argv, "nav_action_server");
	ros::NodeHandle nh;
...
// Basic services
  ...
  Server server(nh, "nav_action_server", boost::bind(&execute, _1, &server), false);
  server.start();  
...
void Mission::step()
{
  ...
  else if(break_counter > 0)
  {
    ...
    controlVelYawRate.axes.push_back(0.5);
    controlVelYawRate.axes.push_back(0.5);
    controlVelYawRate.axes.push_back(0.5);
