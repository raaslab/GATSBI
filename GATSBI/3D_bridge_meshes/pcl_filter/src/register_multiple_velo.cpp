...
void reg_multiple_callback(const sensor_msgs::PointCloud2ConstPtr &input, 
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr output_concat(new pcl::PointCloud<pcl::PointXYZI>);
...
int main(int argc, char **argv) {
    ...
    ros::init(argc, argv, "register_multiple_velo");
    // ros::Rate rate(10);
    ...
    pcl::PointCloud<pcl::PointXYZI> concat;
    boost::shared_ptr<const sensor_msgs::PointCloud2> first_msg = ros::topic::waitForMessage<sensor_msgs::PointCloud2>("transformed_cloud_velo");
    pcl::PointCloud<pcl::PointXYZI>::Ptr temp(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::PCLPointCloud2 pcl_input;
    pcl_conversions::toPCL(*first_msg, pcl_input);
    pcl::fromPCLPointCloud2(pcl_input, concat);
    ros::Subscriber sub = node_handler.subscribe<sensor_msgs::PointCloud2>("transformed_cloud_velo", 1 , 
                                            boost::bind(&reg_multiple_callback, _1, concat));

    pub = node_handler.advertise<sensor_msgs::PointCloud2>("register_multiple_velo", 1);
    ros::spin();
}